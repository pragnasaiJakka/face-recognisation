...
def bayes(x_train, y_train, x_test, y_test):
    ...
    def findCovarinace(x_train):
        ...
        for i in range(0,x_train.shape[1],2):
            if i == 0:
                class_mean_arr = np.mean((x_train[:,i:i+2]),axis = 1).reshape((x_train.shape[0],1))
                continue
            class_mean_arr = np.concatenate((class_mean_arr, np.mean((x_train[:,i:i+2]),axis=1).reshape(x_train.shape[0],1)),axis=1)

        for i in range(0,x_train.shape[1],2):
            
            if i == 0 : 
            
                each_class_mean = class_mean_arr[:,int(i/2)].reshape((x_train.shape[0],1))
                class_i = x_train[:,i:i+2]
...
def main():
    ...
    x_train, mean, eig_vect,_ = PCA_compressed_train(x_train)
    x_test = PCA_compressed_test(x_test, mean, eig_vect)
    test_error = bayes(x_train, y_train, x_test, y_test)
    print('Training Error in PCA compressed data')
    print(test_error)
    print('____')
    
    #MDA analysis
    x_train, y_train, x_test, y_test = DataLoader(face)
    x_train, mean, eig_vect,_ = MDA_train(x_train)
    x_test = MDA_test(x_test, mean, eig_vect)
    test_error = bayes(x_train, y_train, x_test, y_test)
    print('Training Error in MDA compressed data')
    print(test_error)
