...
def PCA_compressed_train(face):
    ...
    reduced_eig_vect = sorted_eig_vect[:, :60]
     
    #forward projection
    P = (reduced_eig_vect.T@shifted_face).real
...
def main():
    ...
    x_train, mean_face, direction,p = PCA_compressed_train(x_train)
    x_test = PCA_compressed_test(x_test, mean_face, direction)
