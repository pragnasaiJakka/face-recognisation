...
def RBF_kernal(x,y, sigma_sqr = 3):
    return np.exp(-(x-y)@(x-y).T/sigma_sqr)


def RBF_SVM(train_data, train_label):
    ...
    for i in range(train_data.shape[0]):
        for j in range(train_data.shape[0]):
            K[i][j] = RBF_kernal(train_data[i], train_data[j])
    ...
    for i in range(SV.shape[0]):
        theta_0 += SV_y[i] - RBF_kernal(SV[i], SV[i])*SV_y[i]*mus[i]
...
def test(test_data, test_label, SV, SV_y, mus, theta_0):
    ...
    for i in range(test_data.shape[1]):
        ...
        for j in range(SV.shape[0]):
            sum += RBF_kernal(image, SV[j])*SV_y[j]*mus[j]
        sum = sum + theta_0
        y_hat = np.sign(sum)
...
def main():
    ...
    SV, SV_y, mus, theta_0 = RBF_SVM(train_data, train_label)
